...
def test_fetch_news_yfinance_maps_fields():
    ...
    from newsletter.news import fetch_news_yfinance
    mock_news = [
        {
    ...
    with patch("newsletter.news.yf.Ticker", return_value=mock_ticker):
...
def test_waterfall_yfinance_before_google():
    ...
    with patch("newsletter.news.fetch_news_marketaux", return_value={}), \
         patch("newsletter.news.fetch_news_yfinance", return_value=yf_result) as mock_yf, \
         patch("newsletter.news.fetch_news_google", return_value={}) as mock_google:
...
def test_google_news_strips_html():
    from unittest.mock import patch, MagicMock
    import feedparser
    from newsletter.news import fetch_news_google
    ...
    with patch("newsletter.news.requests.get") as mock_get, \
         patch("newsletter.news.feedparser.parse", return_value=mock_parsed):
        mock_resp = MagicMock()
        mock_resp.content = b""
