...
- 加 stretch overlay（拉伸 >18% 降杠杆）：60d/180d/360d 前瞻数据完全否定 mean-reversion 假设，**stretch 高之后未来 6-12 个月反而平均涨更多**（Jegadeesh-Titman momentum）。360d ≥18% 样本 91% 正回报、平均 +26.8%
- 结论：择时只用 SMA200 binary 信号；其余均线**仅作展示**，不做花式叠加

### 展示哪些均线
- **SMA50** — 短期趋势
- **SMA100** — 中期趋势
- **SMA200** — 牛熊分水岭（唯一有回测验证的择时线）
- **SMA250** — 约 1 年均线（年度趋势基准）

### 数据流
```
[yfinance Ticker(QQQ/SPY).history(period=500d)] ──→ Close (~350 trading days)
                                                  │
                              ┌──────────────────┴────────────────┐
                              ▼                                    ▼
                    SMA 50/100/200/250 (rolling)            price[-1]
                              │                                    │
                              └─────────────┬──────────────────────┘
                                            ▼
                              MAComparison {
                                 ticker, price,
                                 levels: [MALevel(period, value,
                                          deviation_pct, above), ...]
                              }
                                            │
                                            ▼
...
- 500 自然日 ≈ 350 交易日，给 SMA250 留 ~100 天余量

---

## V1.8：Email Delivery（邮件推送，branch: `claude/eager-einstein-NHXPz`）

### 范围
在现有 Telegram 推送之外，**新增邮件投递通道**。Telegram 和 Email 各自独立开关，可单独或同时启用。复用 `generate_digest()` 输出的同一份 markdown digest，**不改动 pipeline**。

### 设计决策
- **传输方式：SMTP（stdlib `smtplib` + `email`）** — 零新增依赖、免费。支持 Gmail App Password 或任意 SMTP 服务器（Outlook / Fastmail / 企业 relay）。与项目"免费 + 最小依赖"哲学一致（同 Google News RSS / CBOE 的取舍逻辑）。
  - 端口自适应：`SMTP_PORT=465` 走 `SMTP_SSL`；其余端口（如 587）走 `SMTP` + `starttls()`。
- **正文格式：multipart/alternative（HTML + 纯文本）**
  - **纯文本部分** = 原始 digest（markdown 源文本），保证任何邮件客户端 / 纯文本环境可读。
  - **HTML 部分** = 由 digest markdown 轻量转换而来（`##`/`###` → 标题、`**bold**` → `<strong>`、`---` → `<hr>`、空行 → 段落、裸 URL → 链接），套一个内联 CSS 的简洁模板（max-width 600px）。
  - 转换前先 **HTML 转义**（防止催化剂文本里的 `& < >` 破坏结构），再套用格式转换——与 `bot/telegram.py` 现有转义思路一致。
- **收件人**：`EMAIL_TO` 支持逗号分隔多个地址。
- **主题**：取 digest 第一行（去掉 `#` / `**` markdown）作为 subject；解析失败回退到 `Daily Market Digest — <date>`。
- **失败优雅降级**：未配置（`EMAIL_ENABLED=false` 或缺 SMTP 凭据）时打印提示并返回 `False`，**不抛异常、不影响 Telegram 推送**——与 `send_telegram()` 行为完全对齐。

### 新增 / 修改文件
- **新增** `newsletter/email_sender.py`
  - `markdown_to_html(text) -> str` — 轻量 markdown → 内联样式 HTML（纯函数，易单测）
  - `send_email(text, subject=None) -> bool` — 构建 multipart 消息 + SMTP 发送，配置门控
- **修改** `newsletter/config.py` — 新增 `EMAIL_ENABLED / SMTP_HOST / SMTP_PORT / SMTP_USERNAME / SMTP_PASSWORD / EMAIL_FROM / EMAIL_TO`
- **修改** `.env.example` — 新增 email 配置段（含 Gmail App Password 说明注释）
- **修改** `scripts/run_newsletter.py` — 默认 run + `run_scheduled()` job 在 `send_telegram()` 之后追加 `send_email()`
- **修改** `bot/telegram.py` — `_scheduled_digest()` 追加 `await asyncio.to_thread(send_email, digest)`（smtplib 阻塞，放线程避免卡住 event loop）
- **新增** `tests/test_newsletter/test_email_sender.py` — 全 mock，无真实 SMTP / 网络
- **修改** `README.md` — 文档同步

### 不修改
- `newsletter/pipeline.py` — digest 生成逻辑不变
- `requirements.txt` — 纯 stdlib，无新增依赖
...
SMTP_PASSWORD=your_gmail_app_password   # Gmail: 用 App Password，不要用账户密码
EMAIL_FROM=you@gmail.com                # 可选，默认 = SMTP_USERNAME
EMAIL_TO=you@gmail.com,friend@example.com
```

### 数据流变更
```
generate_digest() ──→ digest (markdown)
                          ├──→ send_telegram()   (现有，与 email 无关)
                          └──→ send_email()        (新增，EMAIL_ENABLED 门控)
                                   ├─ text/plain  = digest 源文本
                                   └─ text/html   = markdown_to_html(digest)
```

### 本地测试计划（实现前待 sign-off）
1. `markdown_to_html`：`**bold**` / `##` / `###` / `---` / HTML 转义（`<script>`、`&`）/ 裸 URL 链接化
2. `send_email` 未配置（`EMAIL_ENABLED=false`）→ 返回 `False`，完全不触碰 SMTP
3. `send_email` 已配置 → mock `smtplib.SMTP` & `SMTP_SSL`，断言 `starttls`/`login`/`send_message` 调用、收件人列表正确、返回 `True`
4. multipart 结构：消息同时含 `text/plain` 与 `text/html` part
5. subject 提取：从首行标题正确派生，无标题时回退默认

### API / 成本影响
- 纯 SMTP，无第三方 API 配额；Gmail 免费额度（~500 封/天）远超每日 1 封 digest。
...
| Email 投递 | 除 Telegram 外同时发邮件 → 已提升为 **V1.8**（SMTP，见上） | 低 |
...
- [ ] V1.8: Email 投递（SMTP + HTML/纯文本 multipart）
